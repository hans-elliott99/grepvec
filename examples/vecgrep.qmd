...
t0 = Sys.time()
m <- grep_any(txt[1:500], words)
difftime(Sys.time(), t0)

t0 = Sys.time()
m <- grep_first(txt[1:500], words)
difftime(Sys.time(), t0)


t0 = Sys.time()
m <- grep_first(txt[1:500], words, value = TRUE)
difftime(Sys.time(), t0)


t0 = Sys.time()
m <- grep_count(txt[1:500], words)
difftime(Sys.time(), t0)
...
head(d[grep_any(d$txt_col, pat), ])
head(subset(d, txt_col %grepin% pat))
...
microbenchmark(
    grep_any(x, pat),
    grep_any(as.factor(x), pat)
)
# note - if duplicate patterns, user can call unique on pattern vector first

...
TODO: okay, so the encoding issues are well on their way to being resolved, but translating every string to UTF8 makes grepvec about 2x slower, so that we're back to the speed of base R solutions. Figure out how to speed up!
    - Want to test both needle and haystack vectors for non-ascii strings, in which case we use wchar_t
        - can test X% of each vector and let that be an option
    - If none detected, can proceed with using char
    - Can also add an encoding argument if user wants all strings to be treated as such without translation

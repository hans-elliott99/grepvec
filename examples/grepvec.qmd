...
note - looks good! about as fast as we can get, I think.  
 - actually, very fast! even compared to og using regex.h with no fuss... almost reaches that speed

now, make sure the code is in a good final place and then figure out how to compile on Windows without errors  
...
`grep` is slightly faster for simple use cases, when searching for a single pattern in a single string, which makes sense considering `grepvec` has some extra overhead.  

`grepvec` becomes slightly faster when comparing a pattern with a bunch of strings.  
